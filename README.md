...
`examples/language.smile` and the focused [INPUT example](examples/input.smile) are copied beside the Desktop executable and included in deployment publishes as `language.smile` and `input.smile`. New editor sessions reload only the cumulative runtime reference asynchronously without associating Save with the packaged copy, so learners can experiment safely and use Save As for their own programs.

![SMILE desktop app in maximized state](Requirements/Progress/2026-08-02-day-1-2-smile-desktop.png)

The four code panes use AvalonEdit. The SMILE source pane and all three generated target panes show line numbers and lexical syntax highlighting. `INPUT` is highlighted case-insensitively, and `IF`, `THEN`, `ELSE`, and `END` remain highlighted individually so ELSE IF and END IF visibly remain two keywords. A dedicated Comment style recognizes `REM`, `//`, `#`, and `--` only at first non-whitespace, with case-insensitive boundary-aware REM. Comment and String spans own INPUT-looking text inside comments, ordinary/interpolated Strings, and SET Block Strings. A SET Block String remains one higher-precedence String-colored span across physical lines inside or outside a branch until its whitespace-only closing delimiter; marker-looking content and blank block lines remain String-owned. Unterminated blocks, incomplete comments, and malformed INPUT/IF text remain safe while typing. Target panes switch highlighting when their selected language changes. C++ uses AvalonEdit's built-in C++ definition. Objective-C uses the same mature C/C++ highlighting because SMILE's current Objective-C output is a Foundation-free C-compatible console profile. Language switching reuses generated code already cached for the current source revision and only schedules live transpilation for visible targets that are actually missing. The output area remains a plain build/program log without line numbers.
