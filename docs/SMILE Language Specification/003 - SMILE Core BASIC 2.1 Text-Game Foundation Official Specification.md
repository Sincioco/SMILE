...
Programs that use interactive terminal features initialize only the required console state. Any changed line, echo, cursor, or terminal mode is restored on normal completion, `End Program`, controlled SMILE runtime failure, and normal target exception/error cleanup paths. Redirected execution does not attempt raw-mode setup.

The evaluator exposes an injectable host for one-event key polling, clear/frame capture, virtual Wait, monotonic time, inclusive Random, and an execution budget. Existing ordinary evaluator callers use a safe default host. Exceeding the configured statement budget fails deterministically rather than hanging automated tests.
...
```text
statement       := existing-statement | get-key | clear-screen | wait | random
...
This milestone does not add blocking `Input`, `Key_Held`, pointer/mouse input, cursor-position statements, graphics or `Game Window`, sound, files, dynamic arrays, more than two dimensions, array parameters or returns, `ByRef`, Optional/named/variadic parameters, records, classes, modules, imports, threads in SMILE source, colors, or an eleventh target. Historical LET/SET/INPUT/WHILE/interpolation/block-string syntax remains rejected.
