...
SMILE v0.8.0 [WHILE](009%20-%20SMILE%20-%20WHILE%20Statement%20Official%20Specification%20v1.0.md) reuses these complete-value String rules in conditions, SET values, INPUT, and PRINT inside a loop. Every loop-carried String assignment must reach a finite compile-time maximum UTF-8 byte length under zero-or-more-iteration analysis; unbounded recurrence is rejected by the WHILE specification rather than changing String semantics or guessing a trip count.
...
C and Objective-C may use ordinary `%s` output and `strcmp` equality only for values proven to contain no embedded NUL. A NUL-containing value is emitted through compiler-owned UTF-8 byte data plus an exact byte length. A NUL-sensitive equality may be lowered to a static Boolean result only when branch/loop-aware analysis proves that result on every possible incoming path and iteration; otherwise the target must compare current runtime lengths and bytes. This keeps bytes after NUL observable and remains correct after `SET`, `INPUT`, an IF merge, or a WHILE back-edge changes the current value.
