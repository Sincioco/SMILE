...
Focused generated-target tests use `SmileEvaluator` as the semantic oracle for the active target under test. Tests never trim NUL, backspace, form feed, carriage return, tab, spaces, or meaningful trailing blank lines. All ten targets are active; strict multi-toolchain and warning gates still belong to explicit milestone or release validation rather than routine Velocity Mode.
