...
Generated-target tests use `SmileEvaluator` as the semantic oracle and compare stdout, stderr, and exit code for every installed target. Tests never trim NUL, backspace, form feed, carriage return, tab, spaces, or meaningful trailing blank lines. Compiler-backed targets remain warning-free under the strict release gate.
