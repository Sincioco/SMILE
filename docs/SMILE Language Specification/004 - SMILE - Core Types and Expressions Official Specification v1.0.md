...
Focused generated-expression tests protect the changed active targets and compare ordinary successful behavior with `SmileEvaluator` where that comparison is meaningful. Exact malformed-input bytes, identical host-library diagnostics, and full ten-toolchain execution are not routine language requirements. Broader all-target conformance belongs to milestones, releases, or explicit requests.
