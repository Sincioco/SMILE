...
With blocks use native location aliases or captured indexes on these same toolchains.
Type methods/properties also use these toolchains without additional dependencies.
JavaScript accessors that read files use async getter/setter methods; Swift methods
with overlapping receiver references reuse the existing closure-based location adapter.
