...
Incremental text/routine/Double/file/ByRef additions also record a current authority commit and execute unchanged source fixtures in both implementations. A completed subset must not be presented as full SMILE 2.0 parity; the current back-port inventory distinguishes implemented work, outstanding core features, and graphical features outside the request.
