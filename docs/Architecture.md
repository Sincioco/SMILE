...
SMILE v0.4.2 uses a small direct compiler pipeline:
...
Current `LET` initializers are all compile-time constants because SMILE v0.4.2 has no runtime input or reassignment. The engine evaluates strings, integers, booleans, arithmetic, comparisons, boolean logic, variable references, and interpolated strings once during binding and carries the resulting `SmileValue` on each bound declaration. `AND` and `OR` evaluation is explicitly left-to-right and short-circuiting, while binding still visits both operands so unreachable undefined names and type errors remain diagnostics. C and Objective-C preserve native integer and boolean initializer expressions; COBOL and MASM can use evaluated values directly for declaration storage; high-level targets preserve original initializer intent where the destination language has a natural form.

After binding, one target-independent pass simplifies pure bound expressions. It folds `NOT TRUE` and `NOT FALSE`, removes identity operands from `AND` and `OR`, and replaces the absorbing forms with their Boolean constants. For example, `Adult AND NOT FALSE` becomes the original `Adult` symbol. The pass does not reparse source text, perform target-specific reasoning, or change the declaration's already evaluated `SmileValue`.
...
C and Objective-C emit one safe typed `printf` call for each `PRINT`. Ordinary `int` expressions use `%d`; wide `int64_t` expressions use `%lld` with an explicit value-preserving `long long` cast so the format remains portable across `int64_t` aliases. String expressions use `%s`, booleans use a `TRUE`/`FALSE` conditional argument, literal percent signs are doubled, and embedded NUL text is carried through `%c` so it cannot terminate the format string. Their string equality uses ordinal `strcmp`, with `<string.h>` included only when needed. Simple variables and literals remain natural `strcmp` operands; concatenated or interpolated operands are lowered through their already evaluated `SmileValue` because C has no native String expression value for those forms. COBOL emits canonical text with `DISPLAY`, which avoids fixed-length padding leaks. MASM emits canonical UTF-8 byte labels and writes them with `WriteFile`.
...
The shared simplifier removes literal short-circuit branches such as `FALSE AND ...` and `TRUE OR ...` before every generator. If a short-circuit result depends on a previously declared constant variable rather than a literal, strict destination compilers can still lower the initializer to its already evaluated Boolean value instead of rejecting unreachable constant division.
