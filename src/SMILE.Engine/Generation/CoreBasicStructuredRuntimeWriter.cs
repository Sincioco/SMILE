...
internal static partial class CoreBasicCodeGenerator
{
    private sealed partial class StructuredWriter
    {
        ...
        private void WriteRuntimePreamble()
        {
            switch (_language)
            {
                ...
                case TargetLanguage.Cpp:
                    ...
                    if ((_features.HasMin || _features.HasMax) && _language is TargetLanguage.Cpp)
                    {
        ...
        private void WriteCSharpRuntimeHelpers()
        {
            ...
            {
                Lines("private static void SmileWait(long milliseconds)", "{", "    long remaining = Math.Max(0, milliseconds);", "    while (remaining > int.MaxValue) { System.Threading.Thread.Sleep(int.MaxValue); remaining -= int.MaxValue; }", "    System.Threading.Thread.Sleep((int)remaining);", "}");
            }
            ...
            if (_features.HasRandom)
            {
                Lines("private static long SmileRandom(long lower, long upper)", "{", "    if (lower > upper) throw new ArgumentOutOfRangeException(nameof(upper), \"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\");", "    ulong range = unchecked((ulong)upper - (ulong)lower + 1UL);", "    ulong sample;", "    Span<byte> bytes = stackalloc byte[8];", "    do", "    {", "        System.Security.Cryptography.RandomNumberGenerator.Fill(bytes);", "        sample = BitConverter.ToUInt64(bytes);", "    } while (range != 0 && sample < unchecked(0UL - range) % range);", "    if (range != 0) sample %= range;", "    return unchecked((long)(unchecked((ulong)lower) + sample));", "}");
            }
        }
        ...
        private void WriteCRuntimeHelpers()
        {
            ...
            if (_features.HasWait) Lines("static void smile_wait(int64_t milliseconds)", "{", "    uint64_t remaining = milliseconds > 0 ? (uint64_t)milliseconds : 0;", "    while (remaining > UINT32_MAX) { Sleep(UINT32_MAX); remaining -= UINT32_MAX; }", "    Sleep((DWORD)remaining);", "}");
        ...
        private void WriteCppRuntimeHelpers()
        {
            if (_features.HasGetKey) WriteCGetKeyHelper();
            if (_features.HasClearScreen) WriteCClearHelper();
            if (_features.HasWait) Lines("static void smile_wait(std::int64_t milliseconds)", "{", "    if (milliseconds > 0) std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));", "}");
            ...
            if (_features.HasRandom) Lines("static std::int64_t smile_random(std::int64_t lower, std::int64_t upper)", "{", "    if (lower > upper) throw std::invalid_argument(\"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\");", "    static std::mt19937_64 engine(std::random_device{}());", "    return std::uniform_int_distribution<std::int64_t>(lower, upper)(engine);", "}");
        ...
        {
            Lines("static void smile_clear_screen(void)", "{", "    HANDLE output = GetStdHandle(STD_OUTPUT_HANDLE);", "    DWORD mode;", "    if (output == INVALID_HANDLE_VALUE || !GetConsoleMode(output, &mode)) return;", "    CONSOLE_SCREEN_BUFFER_INFO info;", "    if (!GetConsoleScreenBufferInfo(output, &info)) return;", "    DWORD cells = (DWORD)info.dwSize.X * (DWORD)info.dwSize.Y, written;", "    COORD origin = {0, 0};", "    FillConsoleOutputCharacterA(output, ' ', cells, origin, &written);", "    FillConsoleOutputAttribute(output, info.wAttributes, cells, origin, &written);", "    SetConsoleCursorPosition(output, origin);", "}");
        }
        ...
        private void WriteCRandomHelper(bool cpp = false)
        {
            ...
            Lines($"static {u64} smile_random_state = 0;", $"static {u64} smile_random_bits(void)", "{", "    if (smile_random_state == 0) smile_random_state = ((uint64_t)GetTickCount64() << 1) ^ (uint64_t)(uintptr_t)&smile_random_state ^ UINT64_C(0x9E3779B97F4A7C15);", "    smile_random_state ^= smile_random_state >> 12; smile_random_state ^= smile_random_state << 25; smile_random_state ^= smile_random_state >> 27;", "    return smile_random_state * UINT64_C(2685821657736338717);", "}", $"static {i64} smile_random({i64} lower, {i64} upper)", "{", "    if (lower > upper) { fputs(\"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\\n\", stderr); exit(1); }", $"    {u64} range = ({u64})upper - ({u64})lower + 1;", $"    {u64} sample, threshold = range == 0 ? 0 : (0 - range) % range;", "    do { sample = smile_random_bits(); } while (sample < threshold);", "    if (range != 0) sample %= range;", $"    return ({i64})(({u64})lower + sample);", "}");
        }
        ...
        private void WriteJavaScriptRuntimeHelpers()
        {
            if (_features.HasGetKey)
            {
                Lines("const smileKeyQueue = [];", "let smileInputStarted = false;", "function smileStartInput() {", "    if (smileInputStarted || !process.stdin.isTTY) return;", "    smileInputStarted = true; process.stdin.setRawMode(true); process.stdin.resume();", "    process.stdin.on(\"data\", data => {", "        const map = new Map([[\"w\",1n],[\"W\",1n],[\"a\",2n],[\"A\",2n],[\"s\",3n],[\"S\",3n],[\"d\",4n],[\"D\",4n],[\"\\r\",14n],[\"\\n\",14n],[\"\\u001b\",15n],[\" \",16n],[\"1\",17n],[\"2\",18n],[\"3\",20n],[\"\\t\",21n],[\"4\",22n]]);", "        for (let index = 0; index < data.length; index++) {", "            if (data[index] === 27 && data[index + 1] === 91 && index + 2 < data.length) {", "                smileKeyQueue.push(({65:10n,66:11n,68:12n,67:13n})[data[index + 2]] ?? 19n); index += 2;", "            } else {", "                smileKeyQueue.push(map.get(String.fromCharCode(data[index])) ?? 19n);", "            }", "        }", "    });", "}", "function smileGetKey() { smileStartInput(); return smileKeyQueue.shift() ?? 0n; }", "function smileCleanup() { if (smileInputStarted && process.stdin.isTTY) { process.stdin.setRawMode(false); process.stdin.pause(); smileInputStarted = false; } }");
            }
            if (_features.HasClearScreen) Lines("function smileClearScreen() { if (process.stdout.isTTY) { process.stdout.write(\"\\u001b[2J\\u001b[H\"); } }");
            if (_features.HasWait) Lines("function smileWait(milliseconds) {", "    const delay = milliseconds > 0n ? milliseconds : 0n;", "    const capped = delay > 2147483647n ? 2147483647n : delay;", "    return new Promise(resolve => setTimeout(resolve, Number(capped))).then(() => delay > capped ? smileWait(delay - capped) : undefined);", "}");
            if (_features.HasTimer) Lines("function smileTimer() { return process.hrtime.bigint() / 1000000n; }");
            if (_features.HasRandom) Lines("function smileRandom(lower, upper) {", "    if (lower > upper) throw new RangeError(\"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\");", "    const modulus = 1n << 64n, range = BigInt.asUintN(64, upper - lower + 1n);", "    let sample, threshold = range === 0n ? 0n : (modulus - range) % range;", "    do { sample = randomBytes(8).readBigUInt64LE(); } while (sample < threshold);", "    if (range !== 0n) sample %= range;", "    return BigInt.asIntN(64, BigInt.asUintN(64, lower) + sample);", "}");
        ...
        private void WriteJavaRuntimeHelpers()
        {
            ...
            if (_features.HasWait) Lines("private static void smileWait(long milliseconds)", "{", "    long remaining = Math.max(0, milliseconds);", "    try { while (remaining > 0) { long part = Math.min(remaining, Integer.MAX_VALUE); Thread.sleep(part); remaining -= part; } }", "    catch (InterruptedException interrupted) { Thread.currentThread().interrupt(); }", "}");
            if (_features.HasTimer) Lines("private static long smileTimer() { return System.nanoTime() / 1_000_000L; }");
            if (_features.HasRandom) Lines("private static long smileRandom(long lower, long upper)", "{", "    if (lower > upper) throw new IllegalArgumentException(\"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\");", "    long range = upper - lower + 1, sample, threshold = range == 0 ? 0 : Long.remainderUnsigned(-range, range);", "    do { sample = java.util.concurrent.ThreadLocalRandom.current().nextLong(); } while (Long.compareUnsigned(sample, threshold) < 0);", "    if (range != 0) sample = Long.remainderUnsigned(sample, range);", "    return lower + sample;", "}");
        }
        ...
        {
            if (_features.HasGetKey) Lines("@_silgen_name(\"_kbhit\") func _kbhit() -> Int32", "@_silgen_name(\"_getch\") func _getch() -> Int32", "func smileGetKey() -> Int64 {", "    if _kbhit() == 0 { return 0 }", "    var key = _getch()", "    if key == 0 || key == 224 { if _kbhit() == 0 { return 19 }; key = _getch(); return key == 72 ? 10 : key == 80 ? 11 : key == 75 ? 12 : key == 77 ? 13 : 19 }", "    switch key { case 119,87: return 1; case 97,65: return 2; case 115,83: return 3; case 100,68: return 4; case 13: return 14; case 27: return 15; case 32: return 16; case 49: return 17; case 50: return 18; case 51: return 20; case 9: return 21; case 52: return 22; default: return 19 }", "}");
            ...
            if (_features.HasWait) Lines("func smileWait(_ milliseconds: Int64) { if milliseconds > 0 { Thread.sleep(forTimeInterval: Double(milliseconds) / 1000.0) } }");
            if (_features.HasTimer) Lines("func smileTimer() -> Int64 { Int64(ProcessInfo.processInfo.systemUptime * 1000.0) }");
            if (_features.HasRandom) Lines("func smileRandom(_ lower: Int64, _ upper: Int64) -> Int64 {", "    if lower > upper { fatalError(\"SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.\") }", "    return Int64.random(in: lower...upper)", "}");
        }

        private void WritePythonRuntimeHelpers()
        {
            if (_features.HasGetKey) Lines("smile_extended_key_pending = False", "def smile_get_key():", "    global smile_extended_key_pending", "    if smile_extended_key_pending:", "        if not msvcrt.kbhit():", "            return 0", "        smile_extended_key_pending = False", "        return {'H': 10, 'P': 11, 'K': 12, 'M': 13}.get(msvcrt.getwch(), 19)", "    if not msvcrt.kbhit():", "        return 0", "    key = msvcrt.getwch()", "    if key in ('\\x00', '\\xe0'):", "        if not msvcrt.kbhit():", "            smile_extended_key_pending = True", "            return 0", "        return {'H': 10, 'P': 11, 'K': 12, 'M': 13}.get(msvcrt.getwch(), 19)", "    return {'w': 1, 'W': 1, 'a': 2, 'A': 2, 's': 3, 'S': 3, 'd': 4, 'D': 4, '\\r': 14, '\\n': 14, '\\x1b': 15, ' ': 16, '1': 17, '2': 18, '3': 20, '\\t': 21, '4': 22}.get(key, 19)");
            if (_features.HasClearScreen) Lines("def smile_clear_screen():", "    handle = ctypes.windll.kernel32.GetStdHandle(-11)", "    mode = ctypes.c_ulong()", "    if handle not in (0, -1) and ctypes.windll.kernel32.GetConsoleMode(handle, ctypes.byref(mode)):", "        print('\\x1b[2J\\x1b[H', end='', flush=True)");
            if (_features.HasWait) Lines("def smile_wait(milliseconds):", "    time.sleep(max(0, milliseconds) / 1000)");
            if (_features.HasTimer) Lines("def smile_timer():", "    return time.monotonic_ns() // 1_000_000");
            if (_features.HasRandom) Lines("def smile_random(lower, upper):", "    if lower > upper:", "        raise ValueError('SMILE Runtime Error SMILER1221: Random lower bound exceeds upper bound.')", "    return random.randint(lower, upper)");
        }
