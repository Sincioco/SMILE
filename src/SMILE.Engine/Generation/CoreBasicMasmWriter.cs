...
internal sealed partial class CoreBasicMasmWriter
{
    ...
    public CoreBasicMasmWriter(BoundProgram program)
    {
        ...
        if (_program.Variables.Any(variable =>
            !variable.IsConstant && variable.Type is { Kind: SmileTypeKind.String }))
        {
    ...
    public string Write()
    {
        ...
        foreach (VariableSymbol variable in _program.Variables.Where(item => !item.IsConstant))
        {
            ...
            if (variable.Type is RecordTypeSymbol record)
            {
                Line($"{name} LABEL {_identifiers.Get(record)}");
                Line($"    BYTE {record.NativeSize * (variable.IsArray ? variable.TotalElementCount : 1)} DUP(0)");
                continue;
            }
            if (variable.Type is { Kind: SmileTypeKind.Double })
            {
            ...
            if (variable.IsArray)
            {
                string value = variable.Type is { Kind: SmileTypeKind.String }
    ...
    private void AppendProcedure(ProcedureEmitter procedure)
    {
        ...
        else
        {
            ...
            if (procedure.ReturnOffset is int returnOffset)
            {
                ...
                if (procedure.Routine!.Symbol.ReturnType is { Kind: SmileTypeKind.Double }) Line("    movq xmm0, rax");
            }
            Line("    mov rsp, rbp");
            Line("    pop rbp");
            Line("    ret");
    ...
    private sealed partial class ProcedureEmitter
    {
        ...
        public ProcedureEmitter(CoreBasicMasmWriter owner, BoundRoutineDeclaration routine)
        {
            ...
            {
                _storage[variable] = Allocate(IndirectVariable(variable) ? 8 : NativeValueSize(variable.Type) * (variable.IsArray ? variable.TotalElementCount : 1), variable.TotalElementCount);
            }
        ...
        private void BuildManagedTextLifetime()
        {
            ...
            if (IsMain)
            {
                roots.AddRange(_owner._program.Variables
                    .Where(variable => !variable.IsConstant && variable.Type is { Kind: SmileTypeKind.String })
            ...
            else
            {
                roots.AddRange(Routine!.Locals
                    .Distinct()
                    .Where(variable => !variable.IsByRef && variable.Type is { Kind: SmileTypeKind.String })
                    .Select(variable =>
                    {
        ...
        private void InitializeRoutineFrame()
        {
            ...
            for (int index = 0; index < symbol.Parameters.Count; index++)
            {
                ...
                {
                    Append(_initialization, 1, $"mov rax, QWORD PTR [rbp+{48 + (position - 4) * 8}]");
            ...
            {
                if (variable.Type is RecordTypeSymbol) continue;
                Storage storage = _storage[variable];
                if (variable.IsArray)
                {
                    string defaultOperand = variable.Type is { Kind: SmileTypeKind.String }
            ...
            foreach (BoundSourceItem item in items)
            {
                switch (item)
                {
                    ...
                    case BoundSetStatement set:
                        if (set.Variable.Type is RecordTypeSymbol)
                        {
                            WriteRecordSet(new BoundVariableExpression(set.Variable), set.Value, indent);
                            break;
                        }
                    ...
                    case BoundMemberSetStatement set:
                        WriteRecordSet(set.Target, set.Value, indent);
                        break;
                    ...
                    case BoundReturnStatement returnStatement:
                        ...
                        {
                            if (returnStatement.Value.Type is RecordTypeSymbol)
                            {
                                EmitRecordReturn(returnStatement.Value, indent);
        ...
        {
            if (set.Array.Type is RecordTypeSymbol)
            {
                WriteRecordSet(new BoundArrayExpression(set.Array, set.Indices), set.Value, indent);
                return;
            }
        ...
        private void WriteSelect(BoundSelectStatement select, int indent)
        {
            ...
            if (_owner._usesManagedText && select.Selector.Type is { Kind: SmileTypeKind.String })
            {
            ...
            foreach (BoundSelectCaseClause clause in select.Cases)
            {
                ...
                if (value.Type is { Kind: SmileTypeKind.Double })
                {
                ...
                else if (value.Type is { Kind: SmileTypeKind.String })
                {
                ...
                else
                {
                    ...
                    Emit(indent, $"cmp rax, {(value.Type is { Kind: SmileTypeKind.Boolean } ? (value.BooleanValue ? "1" : "0") : value.IntegerValue.ToString(CultureInfo.InvariantCulture))}");
                    Emit(indent, $"jne {next}");
                }
        ...
        private void EmitPrint(BoundExpression expression, int indent)
        {
            ...
            if (expression.Type is { Kind: SmileTypeKind.Double })
            {
            ...
            if (expression.Type is { Kind: SmileTypeKind.Boolean })
            {
            ...
            Emit(indent, "mov rdx, rax");
            Emit(indent, $"lea rcx, {_owner.InternString(expression.Type is { Kind: SmileTypeKind.Integer } ? "%lld" : "%s")}");
            NoteCall(2);
        ...
        private void EmitExpression(BoundExpression expression, int indent)
        {
            switch (expression)
            {
                ...
                case BoundFieldExpression field:
                    ...
                    if (field.Type is not RecordTypeSymbol) Emit(indent, "mov rax, QWORD PTR [rax]");
                    return;
                case BoundArrayExpression array:
                    if (array.Type is RecordTypeSymbol) { EmitReferenceLocation(array, indent); return; }
                ...
                case BoundUnaryExpression unary:
                    ...
                    if (unary.Operator.Kind is BoundUnaryOperatorKind.Negation) Emit(indent, unary.Type is { Kind: SmileTypeKind.Double } ? "btc rax, 63" : "neg rax");
        ...
        private void EmitBinary(BoundBinaryExpression binary, int indent)
        {
            ...
            if (_owner._usesManagedText && binary.Left.Type is { Kind: SmileTypeKind.String })
            {
            ...
            if (_owner._usesManagedText && binary.Right.Type is { Kind: SmileTypeKind.String })
            {
        ...
        private void EmitCall(RoutineSymbol routine, IReadOnlyList<BoundExpression> arguments, int indent, IReadOnlyList<int>? parameterOrder)
        {
            ...
            for (int sourceIndex = 0; sourceIndex < arguments.Count; sourceIndex++)
            {
                ...
                if (!byRef && argument.Type is RecordTypeSymbol record)
                {
                    ...
                    {
                        Storage copy = NewRecordTemporary(record);
                        Emit(indent, $"lea rcx, {Address(copy.Offset)}");
                        EmitRecordCopy(record, indent);
                        Emit(indent, $"lea rax, {Address(copy.Offset)}");
                    }
                    else Emit(indent, "mov rax, QWORD PTR [rax]");
                }
                Storage temporary = NewTemporary();
                Emit(indent, $"mov QWORD PTR {Address(temporary.Offset)}, rax");
            ...
            for (int index = 0; index < captured.Count; index++)
            {
                ...
                else
                {
                    ...
                    Emit(indent, $"mov QWORD PTR [rsp+{32 + (position - 4) * 8}], rax");
                }
            }
            ...
            {
                if (shift == 0) Emit(indent, $"mov QWORD PTR {Address(recordResult.Offset)}, rax");
                Emit(indent, $"lea rax, {Address(recordResult.Offset)}");
            }
        }

        private void EmitBoundsCheck(VariableSymbol array, int dimension, int indent)
            => EmitBoundsCheck(array.ArrayDimensions[dimension], indent);

        private void EmitBoundsCheck(int length, int indent)
        {
